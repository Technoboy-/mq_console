    ...
    public String updateKvConfig(ModelMap map, HttpServletRequest request,
        ...
        try {
            if (request.getMethod().equals(GET)) {
                ...
                clientConfig.setNamesrvAddr(configureInitializer.getNamesrvAddr());
                clientConfig.setInstanceName(String.valueOf(System.currentTimeMillis()));
                String clientId = clientConfig.buildMQClientId();
                mqClientInstance = new MQClientInstance(clientConfig, new Long(System.currentTimeMillis() / 1000).intValue(), clientId);
                mqClientInstance.start();
                MQClientAPIImpl mqClientAPI = mqClientInstance.getMQClientAPIImpl();
                mqClientAPI.putKVConfigValue(namespace,key,value,3000);
                putAlertTrue(map);
            }
            else {
